...
const AppContext = createContext({});
  ...
  export const AppProvider = ({ children }) => {
    ...
    const [allempolyee,setallemploye]=useState([])
    ...
    const countryListAllIsoData = [
        ...
        {"code": "VG", "code3": "VGB", "name": "Virgin Islands", "number": "092"},
    ...
    function enableaddform(){
        document.getElementById('addemploye-form').style.display='flex'
    }

    function handlecancel(){
        document.getElementById('addemploye-form').style.display='none'
    }
    async function postthedata(){
        const url='http://localhost:8080/employe';
        try{
            await axios.post(url,postingdata).then((res)=>{
    ...
    const updateemploye=(feild,value)=>{
        setselectedemployee((perv)=>({
            ...perv,[feild]:value,
        }))
    }
    

    function handletablerow(empl){
    ...
    async function actionemploye(e){
        e.preventDefault()
        const url='http://localhost:8080/updateemploye';
        try{
            await axios.put(url,selectedemployee).then((res)=>{
    ...
    async function handledelete(selectedemployee){
        const del=selectedemployee.employeid
        const url=`http://localhost:8080/deleteemploye/${del}`;
        try{
    ...
    function searchtheemploye(e){
        
        const serach=e.target.value.toLowerCase();
        
        const filtereduser=allempolyee.filter(employe=>employe.name.toLowerCase().includes(serach)
                            || employe.name.toLowerCase().includes(serach)
                            || employe.gender.toLowerCase().includes(serach)
                            || employe.role.toLowerCase().includes(serach)
                            || employe.department.toLowerCase().includes(serach)
                            || employe.country.toLowerCase().includes(serach)
                            || employe.employeid.toString().toLowerCase().includes(serach)                       
        )
    ...
    return (
      <AppContext.Provider value={{filtereddata,searchtheemploye,updateemploye,selectedemployee,isloading,allempolyee,updatefield,postingdata,countryListAllIsoData,newemployee,enableaddform,handlecancel,handlecancel1,handletablerow,actionemploye,handledelete}}>
        {children}
      </AppContext.Provider>
    );
  };
